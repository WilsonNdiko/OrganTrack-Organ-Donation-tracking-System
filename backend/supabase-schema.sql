...
CREATE TRIGGER ledger_events_updated_at
...
CREATE INDEX IF NOT EXISTS ledger_events_event_type_idx ON public.ledger_events(event_type);
CREATE INDEX IF NOT EXISTS ledger_events_organ_id_idx ON public.ledger_events(organ_id);
CREATE INDEX IF NOT EXISTS ledger_events_event_id_idx ON public.ledger_events(event_id);
...
CREATE POLICY "Enable all operations for organ_requests" ON public.organ_requests
    FOR ALL USING (true) WITH CHECK (true);

CREATE POLICY "Enable all operations for ledger_events" ON public.ledger_events
    FOR ALL USING (true) WITH CHECK (true);
